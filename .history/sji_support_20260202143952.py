...
def plot_sji(sjimap, show_time = True, log = True, mode = 'extent', **kwargs):
    ...
    if mode == 'extent':
        ...
        if log:
            im = ax.imshow(sjidata, cmap = cmap, norm = colors.LogNorm(sat[0], sat[1]), extent = sjiextent, origin = 'lower', aspect = 'auto')      
        else: 
            im = ax.imshow(sjidata, cmap = cmap, vmin = sat[0], vmax = sat[1], extent = sjiextent, origin = 'lower', aspect = 'auto')     
        ...
        if log:
            im = ax.imshow(sjidata, cmap = cmap, norm = colors.LogNorm(sat[0], sat[1]), aspect = 'auto')
        else: 
            im = ax.imshow(sjidata, cmap = cmap, vmin = sat[0], vmax = sat[1], aspect = 'auto')      
