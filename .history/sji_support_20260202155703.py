...
def diffrot(in_map, out_time):
    ...
    with propagate_with_solar_surface():
        out_map = in_map.reproject_to(out_wcs, preserve_date_obs=True)#, rotation+)
...
def plot_sji(sjimap, show_time = True, log = True, aspect = 'auto', mode = 'extent', **kwargs):

    # caution: aspect = 'auto' will force the image to a fixed aspect ratio depending on the size of the axis box. 
    # use aspect = 'equal' to preserve original dimensions of the plotted image
    
    if 'cmap' in kwargs:               
        cmap_string = kwargs.get('cmap', 'irissjiFUV')
    ...
    if mode == 'extent':
        ...
        if log:
            im = ax.imshow(sjidata, cmap = cmap, norm = colors.LogNorm(sat[0], sat[1]), extent = sjiextent, origin = 'lower', aspect = aspect)      
        else: 
            im = ax.imshow(sjidata, cmap = cmap, vmin = sat[0], vmax = sat[1], extent = sjiextent, origin = 'lower', aspect = aspect)     
        ...
        if log:
            im = ax.imshow(sjidata, cmap = cmap, norm = colors.LogNorm(sat[0], sat[1]), aspect = aspect)
        else: 
            im = ax.imshow(sjidata, cmap = cmap, vmin = sat[0], vmax = sat[1], aspect = aspect)      
